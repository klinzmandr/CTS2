...
//	echo "apply new password to database<br>";
	$sql = "UPDATE `cts2users` SET `UserID`='$userid', `Password`='$password1' WHERE `UserID`='$userid' AND `Password`='$password'";
//	echo "SQL: $sql<br>";
	$res = doSQLsubmitted($sql);	// do update
	
	$sql = "SELECT * FROM `cts2users` WHERE `UserID`='$userid' AND `Password`='$password1'";
...
//	echo "<pre>user: "; print_r($r); echo "</pre>";
	if ($nbrofrows == 0) {
		echo "ERROR: userid and/or password provided not applied.<br>";
		}
	else {
...
function checkflds(form) {
	...
	var emailFilter = /^[^@]+@[^@.]+\.[^@]*\w\w$/ ;
	var illegalChars= /[\(\)\<\>\,\;\:\\\"\[\]]/ ; 			// " comment to close quotes
